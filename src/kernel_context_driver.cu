...
#include <overlap_index_switch.cu>
...
int main() {
    ...
    MicrobenchmarkDriver<OverlappedIndexSwitchKernel<vt, int, 1, 32>> overlapped_index_switch_driver(N, bs_vec, output_dir+"overlapped_index_switch_1_32_kernel_output.csv", &dev_ctx, true);
    if (!overlapped_index_switch_driver.check_then_run_kernels()) {return -1;} 
    total_runs += overlapped_index_switch_driver.get_total_runs();
